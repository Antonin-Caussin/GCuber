...
calculate_volumes <- function(df, volume_type = "V22",
  ...
  validate_parameters <- function() {
    ...
    if (length(missing_columns) > 0) {
      cat("[DEBUG] ⚠️  Missing columns:", paste(missing_columns, collapse = ", "), "\n")
    }
  ...
  establish_species_correspondence <- function(df) {
    ...
    if (length(values_without_correspondence) > 0) {
      cat("[DEBUG] ⚠️  Values without correspondence (", length(values_without_correspondence), "):",
  ...
  convert_circumference <- function(df_result, C150, C130, from_col = NULL, to_col = NULL, direction = NULL) {
    ...
    cat("[DEBUG] ✅ Successful", from_col, "→", to_col, "conversions:", successful_conversions, "\n")
    if (attempted_conversions > successful_conversions) {
      cat("[DEBUG] ⚠️  Failed conversions:", attempted_conversions - successful_conversions, "\n")
    }
